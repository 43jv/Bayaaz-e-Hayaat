              ...
              {filteredCards.map((card) => (
                <Grid item key={card.id} xs={12} sm={6} md={4}>
                  ...
                  >
                    ...
                    <CardActions>
                    <Link
                    to={`/RenderDoc/${encodeURIComponent(card.image)}${card.digitisedText ? `,${encodeURIComponent(card.digitisedText)}` : ""}?title=${encodeURIComponent(card.title)}&engInfo=${encodeURIComponent(card.engInfo)}&urduInfo=${encodeURIComponent(card.urduInfo)}`}
